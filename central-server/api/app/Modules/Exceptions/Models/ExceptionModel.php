...
class ExceptionModel extends Database
{
    ...
    {
        try {
            $sql = "SELECT ex.*, CONCAT(u.fname, ' ', u.lname) AS created_by_name FROM tbl_exception ex LEFT JOIN tbl_user u ON ex.created_by = u.id WHERE ex.id = ?";
    ...
    {
        try {
            $sql = "SELECT ex.*, CONCAT(u.fname, ' ', u.lname) AS created_by_name FROM tbl_exception ex LEFT JOIN tbl_user u ON ex.created_by = u.id";
            $params = [];
            if ($exception_type) {
